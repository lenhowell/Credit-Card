...
struct DescriptionKey {
    ...
    // if only one word:
    static internal func stripTrailingNumber(_ keyIn: String, fileName: String = "") -> String {
        var key = keyIn
        var key2 = key
    ...
    static func checkDif(newStr: String, oldStr: String, doPrint: Bool, comment: String) -> String {
        if oldStr != newStr {
            ...
            dictDescKeyAlgorithmCnts[comment, default: 0] += 1
            return newStr
        }
        return oldStr
    }//end func checkDif
    
}//end struct DescriptionKey
